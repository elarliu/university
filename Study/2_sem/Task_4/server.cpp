...
{ 
    public:
        MyServerSocket () : InServerSocket (PORT_NUM) {} protected:
...
int main(int argc, char* argv[]) 
{
    try 
    {
    // создаём серверный сокет MyServerSocket sock;
        for (;;)
        {
        // слушаем запросы на соединение sock.Accept();
        } 
    }
    catch (Exception& e) 
    {
    // ошибка --- выводим текст сообщения на экран e.Report();
    }
    return 0; 
}